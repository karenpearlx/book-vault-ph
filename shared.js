...
const BVPH = (() => {
  ...
  async function loadBooks() {
    ...
    const { data, error } = await sb.from('books').select('id,title,author,genre,condition,price_bought,price_sell,summary,is_featured,status,rating,goodreads_url,cover_url,created_at,has_adaptation').order('created_at', { ascending: false });
    if (error) {
